...
class DigitalSalesProfiler:
    ...
    async def run_single_test(self, config: str, mode: str, task_name: str) -> TestResult:
        ...
        if trace_id:
            ...
            if result.langfuse_data:
                print(f"✓ Langfuse data retrieved successfully")
            else:
                print(f"⚠ Could not retrieve complete Langfuse data")
    ...
    async def _extract_langfuse_data(self, trace_id: str, max_retries: int = 10, initial_delay: float = 2.0) -> Optional[Dict[str, Any]]:
        """
        ...
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 404:
                        if attempt < max_retries - 1:
                            print(f"  Trace not yet available (attempt {attempt + 1}/{max_retries}), waiting {delay:.1f}s...")
                    ...
                    if not data.get('observations') or len(data.get('observations', [])) == 0:
                        if attempt < max_retries - 1:
                            print(f"  Trace data incomplete (no observations yet, attempt {attempt + 1}/{max_retries}), waiting {delay:.1f}s...")
                            await asyncio.sleep(delay)
                            delay *= 1.5
                            continue
                        else:
                    ...
                    print(f"  ✓ Langfuse data fetched successfully ({len(data.get('observations', []))} observations)")
                    return data
                ...
                if attempt < max_retries - 1:
                    print(f"  Error fetching data (attempt {attempt + 1}/{max_retries}): {e}, retrying in {delay:.1f}s...")
                    await asyncio.sleep(delay)
                    delay *= 1.5
                    continue
                else:
                    print(f"  Warning: Could not fetch Langfuse data for trace {trace_id} after {max_retries} attempts: {e}")
                    return None
